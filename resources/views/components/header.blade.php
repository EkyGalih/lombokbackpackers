<div class="fixed top-0 w-full z-50 bg-white/90 backdrop-blur border-b border-gray-200 shadow">
...
<section class="relative h-44 overflow-hidden">
    ...
    {{-- <div class="relative z-10 h-full flex items-start justify-start">
        <div x-data="{ show: false }" x-init="setTimeout(() => show = true, 200)" x-show="show"
            ...
            class="p-8 md:p-16">
            <h1 class="text-2xl sm:text-3xl md:text-6xl font-extrabold mb-2 text-white drop-shadow-lg">
                {{ $title }}
            </h1>
            @if ($breadcrumb != null)
                <p class="text-[10px] sm:text-xs md:text-sm mb-4 font-semibold text-white drop-shadow">
                    Home > {{ $breadcrumb }}
                </p>
            @endif
        </div>
