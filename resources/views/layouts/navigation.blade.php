        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('bookings.index')" :active="request()->routeIs('my-bookings')">
                        {{ __('My Booking') }}
                    </x-nav-link>
                </div>
            </div>
                        ...
                        <ul class="space-y-2">
                            @forelse (auth()->user()->notifications->take(5) as $notification)
                                <li class="bg-white rounded shadow p-4">
                                    <p>{{ $notification->data['tour_title'] ?? '-' }} - Status:
                                        <strong>{{ strtoupper($notification->data['status']) }}</strong>
                                    </p>
                                    <small
                                        class="text-gray-500">{{ $notification->created_at->diffForHumans() }}</small>
                                </li>
                            @empty
                                <li class="text-gray-500 px-4 py-2">No notifications</li>
                            @endforelse
                        </ul>
                    </x-slot>
                </x-dropdown>
