<div x-data="{
    ...
    <div
        ...
        <div class="flex items-center gap-2">
            ...
            @if ($isMultiple)
                ...
                @else
                    <p class="text-xs">
                        {{ trans('curator::views.panel.add_multiple_file', ['key' => config('curator.multi_select_key')]) }}
                    </p>
                @endif
            @endif
        </div>
    ...
    ">
                    ...
                    <li wire:key="media-{{ $file['id'] }}" class="relative aspect-square"
                        ...
                        <button type="button" x-on:click="handleItemClick('{{ $file['id'] }}', $event)"
                            ...
                            @if (str_contains($file['type'], 'image'))
                                <img src="{{ $file['thumbnail_url'] }}" alt="{{ $file['alt'] ?? '' }}" width="300"
                                    height="300" class="block w-full h-full checkered" />
                            @else
                                <div
        ...
        <!-- Sidebar -->
        <div
            class="curator-panel-sidebar w-full lg:h-full lg:max-w-xs overflow-auto bg-gray-100 dark:bg-gray-900/30 flex flex-col shadow-top lg:shadow-none z-[1]">
            <div class="flex-1 overflow-hidden">
                <div class="flex flex-col h-full overflow-y-auto">
                    ...
                    <div
                        class="flex items-center justify-start mt-auto gap-3 py-3 px-4 border-t border-gray-300 bg-transparent dark:border-gray-800 dark:bg-black/10">
                        @if (count($selected) !== 1)
                            <div>
